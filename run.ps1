...
param(
    [ValidateSet("cli", "web", "voice", "rest", "telegram", "mcp", "hud", "spotlight", "tray", "--help", "-h")]
    [string]$Mode = "cli",
...
if ($Mode -eq "--help" -or $Mode -eq "-h") {
    ...
    Write-Host "  spotlight Floating Cyberpunk spotlight search bar"
    Write-Host "  tray      System tray companion icon & telemetry"
    Write-Host ""
