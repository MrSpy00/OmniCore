...
class WebBypassScraper(BaseTool):
    ...
    async def execute(self, tool_input: ToolInput) -> ToolOutput:
        ...
        try:
            async with httpx.AsyncClient(timeout=20, headers=headers, follow_redirects=True, verify=True) as client:
...
class WebExtractAllEmails(BaseTool):
    ...
    async def execute(self, tool_input: ToolInput) -> ToolOutput:
        ...
        try:
            async with httpx.AsyncClient(timeout=20, follow_redirects=True, verify=True) as client:
                response = await client.get(url)
                response.raise_for_status()
