...
def _spotify_control(
    ...
    if action_norm in ("search", "play_track", "search_and_play") or (
        ...
        # SECURITY: shell=False prevents command injection via uri
        subprocess.run(["cmd", "/c", "start", "", uri], shell=False, capture_output=True, timeout=10)
        time.sleep(1.2)
...
def _launch_windows_app(app: str) -> None:
    ...
    # 1. Try URI scheme via cmd start — most reliable for UWP.
    uri = uri_map.get(app_lower)
    if uri:
        res = subprocess.run(["cmd", "/c", "start", "", uri], shell=False, check=False)
        if res.returncode == 0:
            return

    # 2. Try direct start command (works for exe-based apps).
    res = subprocess.run(["cmd", "/c", "start", "", app], shell=False, check=False)
    if res.returncode == 0:
        return
    ...
    try:
        if shutil.which(app):
            subprocess.run(["cmd", "/c", "start", "", app], shell=False, check=False)
            return
    except Exception:
        pass
