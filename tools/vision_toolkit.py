...
def _click_with_self_correction(
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        temp_path = Path(pathlib_temp_dir()) / f"vision_verify_{uuid.uuid4().hex[:8]}.png"
...
class VisionInstantScreenContext(BaseTool):
    ...
    async def execute(self, tool_input: ToolInput) -> ToolOutput:
        ...
        def _worker() -> dict[str, Any]:
            temp_img = Path(pathlib_temp_dir()) / f"omnicore_instant_{uuid.uuid4().hex[:8]}.png"
            try:
                ...
                # Retain or remove based on needs, but ensure file handle is clean
                pass
...
class VisionSetOfMarkAnnotate(BaseTool):
    ...
    async def execute(self, tool_input: ToolInput) -> ToolOutput:
        ...
        default_som = Path(pathlib_temp_dir()) / f"som_annotated_{uuid.uuid4().hex[:8]}.png"
        output_file = _resolve_sandboxed(str(out_param)) if out_param else default_som

        def _worker() -> dict[str, Any]:
            from PIL import ImageDraw

            temp_raw = Path(pathlib_temp_dir()) / f"som_raw_{uuid.uuid4().hex[:8]}.png"
