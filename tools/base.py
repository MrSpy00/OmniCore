...
def resolve_user_path(path_str: str) -> tuple[Path, bool]:
    ...
    - Path traversal attempts (``../../etc/passwd``) are blocked for relative paths.
    ...
    # Use realpath to resolve symlinks and verify path stays within home directory
    normalized = raw.replace("\\", "/")

    # Block path traversal attempts early
    if ".." in normalized.split("/"):
        # SECURITY: Use realpath to prevent TOCTOU race condition
        home_resolved = os.path.realpath(str(home))
        candidate = os.path.realpath(os.path.join(str(home), raw))
        if not candidate.startswith(home_resolved + os.sep) and candidate != home_resolved:
            raise PermissionError(f"Path traversal detected: '{path_str}' resolves outside home directory")
    ...
    if alias_path is not None:
        # SECURITY: Verify alias path doesn't escape home
        home_resolved = os.path.realpath(str(home))
        alias_resolved = os.path.realpath(str(alias_path))
        if not alias_resolved.startswith(home_resolved + os.sep) and alias_resolved != home_resolved:
            raise PermissionError(f"Alias path traversal detected: '{path_str}' resolves outside home directory")
        return Path(alias_resolved), False

    # SECURITY: Verify final path doesn't escape home
    home_resolved = os.path.realpath(str(home))
    final_path = os.path.realpath(os.path.join(str(home), raw))
    if not final_path.startswith(home_resolved + os.sep) and final_path != home_resolved:
        raise PermissionError(f"Path traversal detected: '{path_str}' resolves outside home directory")
    return Path(final_path), False
