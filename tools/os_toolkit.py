...
def _attempt_windows_elevation_write(path: Path, content: str) -> dict[str, object]:
    """Attempt elevated write by safely staging content in a temp file and copying with RunAs -Wait.

    Security: Uses temp file with -File parameter to prevent command injection.
    Content is staged in a temp file to avoid command-line length limits.
    Strict path validation prevents traversal attacks.
    """
    import tempfile

    # Security: Validate content size to prevent abuse
    max_content_size = 10 * 1024 * 1024  # 10MB limit
    if len(content) > max_content_size:
        return {"ok": False, "error": f"Content exceeds maximum size of {max_content_size} bytes"}

    # Security: Validate path - reject paths with suspicious characters
    ...
    # Only allow writes to user-writable locations
    user_root = _home_root().resolve()
    try:
        target_path.relative_to(user_root)
    except ValueError:
        return {"ok": False, "error": f"Elevation only allowed within user root: {user_root}"}

    temp_staged = None
    ps_file = None
    try:
        # Stage content in a secure temp file
        temp_staged = tempfile.NamedTemporaryFile(delete=False, suffix=".tmp", mode="w", encoding="utf-8")
        temp_staged.write(content)
        temp_staged.close()
        staged_path = Path(temp_staged.name).resolve()

        # Build PowerShell script using -LiteralPath for safe path handling
        # No string interpolation of paths into script body
...
$resolvedDest = Resolve-Path -LiteralPath $DestinationPath -ErrorAction Stop

Copy-Item -LiteralPath $resolvedSource -Destination $resolvedDest -Force

if (-not (Test-Path -LiteralPath $resolvedDest -PathType Leaf)) {
    throw 'Write verification failed'
}

Write-Output "Elevation write completed successfully"
"""

        # Write PowerShell script to temp file
        ps_file = tempfile.NamedTemporaryFile(  # noqa: SIM115
            delete=False, suffix=".ps1", mode="w", encoding="utf-8"
        )
        ps_file.write(ps_script)
        ps_file.close()

        # Use Start-Process with -Wait for elevation, but call powershell -File directly
        elevated_cmd = [
            "powershell.exe",
            "-NoProfile",
            "-NonInteractive",
            ...
            f"'-File', '{ps_file.name}', "
            f"'-SourcePath', '{str(staged_path)}', "
            f"'-DestinationPath', '{str(target_path)}') "
        ...
        # Verify target was successfully written
        success = target_path.exists() and target_path.is_file()
        return {
            "ok": success,
            "stdout": completed.stdout.strip(),
            "stderr": completed.stderr.strip(),
            "returncode": completed.returncode if success else 1,
        }
    except subprocess.TimeoutExpired:
        return {"ok": False, "error": "Elevation timed out after 120 seconds"}
    except Exception as exc:
        return {"ok": False, "error": str(exc)}
    finally:
        # Clean up temp files
        for temp_file in (temp_staged, ps_file):
