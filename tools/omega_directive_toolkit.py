...
def _kill_connections_sync(host: str, port: int) -> dict[str, object]:
    ...
    # Validate host: must be a valid IP or hostname (no shell metacharacters)
    _safe_host_re = _re.compile(r"^[a-zA-Z0-9._:-]+$")
    if host and not _safe_host_re.match(host):
        return {
            "backend": "blocked",
            "returncode": 1,
            "stderr": f"Invalid host format: {host}",
            "host": host,
            "port": port,
        }
    ...
    cmd = ["sh", "-lc"]
    if host and port > 0:
        expr = f"ss -tpn | grep {shlex.quote(host + ':' + str(port))} | awk '{{print $7}}'"
    elif host:
        expr = f"ss -tpn | grep {shlex.quote(host)} | awk '{{print $7}}'"
    else:
