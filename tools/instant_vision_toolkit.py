...
class InstantScreenContext(BaseTool):
    ...
    async def execute(self, tool_input: ToolInput) -> ToolOutput:
        ...
        try:
            ...
            analysis_text = ""
            try:
                from core.llm import LLMFactory  # noqa: I001
                from langchain_core.messages import HumanMessage  # noqa: I001

                llm = LLMFactory.create_chat_model()

                # Read image bytes asynchronously and encode as base64
                raw_bytes = await asyncio.to_thread(Path(saved_path).read_bytes)
