...
_PROMPT_STYLE = Style.from_dict(
    {
        ...
        "completion-menu": "bg:#0f172a #f1f5f9",
        ...
        "completion-menu.meta.completion.current": "bg:#3b82f6 #ffffff bold",
    ...
    class OmniCompleter(Completer):
        ...
        def get_completions(self, document, complete_event):
            try:
                ...
                if cmd == "/perm":
                    options = [
                        ("full", "Tam Yetki (tüm işlemler otomatik onaylanır)"),
                        ("safe", "Güvenli Mod (zararsız otomatik, kritikler sorar)"),
                    ...
                    # Dynamic: show all supported providers with API key status
                    try:
                        from config.live_config import get_live_config
                        from config.settings import get_available_models, get_settings

                        settings = get_settings()
                        live = get_live_config()
                        current = (live.get("provider") or settings.llm_provider or "").strip().lower()
                        for prov in settings.provider_preference:
                            models = get_available_models(prov)
                            if not models:
                                continue
                            has_key = settings.provider_availability.get(prov, False)
                            ...
                            yield Completion(
                                ...
                                display=prov,
                                display_meta=f"{key_icon} {len(list(models.values())[0])} modeller{marker}",
                            )
                    except Exception:
                        # Fallback static list
                        for prov in ["gemini", "groq", "openai", "anthropic", "deepseek", "mistral", "ollama"]:
                            if prov.startswith(sub_text.lower()):
                                yield Completion(prov, start_position=-len(sub_text), display=prov)

                elif cmd == "/setmodel":
                    # Dynamic: show models for current provider
                    try:
                        from config.live_config import get_live_config
                        from config.settings import get_available_models, get_settings
                        ...
                        settings = get_settings()
                        live = get_live_config()
                        current_prov = (live.get("provider") or settings.llm_provider or "").strip().lower()
                        current_model = (live.get("model") or "").strip()
                        provider_models = get_available_models(current_prov)
                        for prov, models in provider_models.items():
                            for m in models:
                                mid = m["id"]
                                name = m.get("name", mid)
                                marker = " *" if mid == current_model else ""
                                # Show short aliases for common models
                                short = mid.split("/")[-1] if "/" in mid else mid
                                yield Completion(
                                    short,
                                    start_position=-len(sub_text),
                                    display=short,
                                    display_meta=f"{name}{marker}",
                                )
                    except Exception:
                        for alias, desc in [
                    ...
                    for opt, desc in [
                        ("help", "Taste kullanımı hakkında bilgi"),
                        ("reset", "Öğrenilmiş tercihleri sıfırla"),
                    ...
                    for opt, desc in [
                        ("short", "Kısa süreli bellek önizleme"),
                        ("long", "Uzun süreli bellek önizleme"),
                        ("graph", "Graf bellek durumu"),
                        ("stats", "Bellek istatistikleri"),
                    ]:
                        if opt.startswith(sub_text.lower()):
                            yield Completion(opt, start_position=-len(sub_text), display=opt, display_meta=desc)
                    ...
                    for opt, desc in [
                        ("auto", "Otomatik commit mesajı oluştur"),
                        ("status", "Git durumunu göster"),
                    ]:
                        if opt.startswith(sub_text.lower()):
                            yield Completion(opt, start_position=-len(sub_text), display=opt, display_meta=desc)
...
def _enable_ansi_windows() -> None:
    ...
    try:
        ...
        subprocess.run(["cmd", "/c", "chcp", "65001"], capture_output=True, shell=False, timeout=2)  # nosec B603, B607
    except Exception:
        pass
...
def _read_key_unix() -> str | None:
    """Read a single keypress on Unix/macOS with timeout to avoid ESC hang."""
    import select
    import sys
    import termios
    import tty
    ...
    try:
        ...
        ch = sys.stdin.read(1)
        if ch == "\x1b":
            # Check if an escape sequence follows within 50ms
    ...
    try:
        ...
        def _render():
            ...
            try:
                ...
                _lc = get_live_config()
                _st = get_settings()
                _prov = (_lc.get("provider") or _st.llm_provider or "").strip().upper()
                _mdl = (_lc.get("model") or "").strip()
                _hdr = f" {_prov}"
                if _mdl:
                    _hdr += f" | {_mdl}"
            except Exception:
            ...
            lines.append(f"  \033[1;36m│\033[0m  \033[1;97m📋 Komut Menüsü\033[0m\033[90m{_hdr}\033[0m")
            lines.append("  \033[1;36m├──────────────────────────────────────────────┤\033[0m")
            ...
            for cat_name, cmds in _SLASH_CATEGORIES:
                ...
                for cmd, desc in cmds:
                    if flat_idx == idx:
                        marker = "\033[44m \033[0m\033[1;97m ▸\033[0m"
                    else:
                        marker = "  "
                    lines.append(f"  \033[1;36m│\033[0m{marker} \033[1;97m{cmd:<12}\033[0m \033[90m{desc}\033[0m")
                    flat_idx += 1
...
def _get_banner() -> str:
    ...
    provider = (live_config.get("provider") or settings.llm_provider or "").strip().lower() or "gemini"
    model = (
        live_config.get("model")
        or getattr(settings, f"{provider}_primary_model", None)
        or getattr(settings, f"{provider}_model", None)
        or settings.omni_llm_model
    )
...
class CLIGateway:
    ...
    def _init_prompt_session(self) -> None:
        ...
        try:
            ...
            def _(event):
                ...
                buf = event.current_buffer
                text = getattr(buf, "text_before_cursor", "") or ""
                if text.startswith("/"):
            ...
            def _(event):
                ...
                if cmd_root in COMMANDS_REQUIRING_ARGS and len(text.split()) <= 1:
                    try:
                        ...
                        pass
                    if not (getattr(buf, "text", "") or "").endswith(" "):
                        buf.insert_text(" ")
                    return
                ...
                if selected:
                    ...
                    if cmd_root in COMMANDS_REQUIRING_ARGS and len(selected.strip().split()) == 1:
                        ...
                        try:
                            ...
                            raw_arg = await asyncio.to_thread(input, f"  {cmd_root} > ")
                            arg_input = raw_arg.strip()
                            if arg_input:
    ...
    def _print_sysinfo(self) -> None:
        ...
        print("─" * 58)
        is_local = str(provider).lower() in ("ollama", "local", "vllm")
        privacy_status = (
            "%100 YEREL (Cihaz dışına veri aktarılmaz)" if is_local else f"BULUT ({str(provider).upper()} API)"
        )
        print(f"  🔒 Gizlilik Durumu : {privacy_status}")
        print("═" * 58 + "\n")
    ...
    async def _handle_config_command(self, user_input: str) -> None:
        ...
        if cmd == "/config":
            ...
            if not subcmd or subcmd not in ("full", "safe", "ask"):
                ...
                try:
                    choice_raw = await asyncio.to_thread(input, "\nSeçiminiz (1/2/3 veya full/safe/ask, Enter=iptal): ")
    ...
    def _refresh_settings_on_router(self) -> None:
        ...
        try:
            ...
            # Update _runtime_provider so _build_llm uses the correct provider path
            new_provider = (new_settings.llm_provider or "").strip().lower()
            if new_provider:
                self._router._runtime_provider = new_provider
    ...
    def _get_live_overrides(self) -> dict[str, Any]:
        ...
        provider = (live_config.get("provider") or "").strip().lower()
        if provider:
            overrides["llm_provider"] = provider
        if live_config.get("model"):
            model_val = live_config.get("model")
            if provider == "groq":
                overrides["groq_primary_model"] = model_val
            else:
                overrides["omni_llm_model"] = model_val
