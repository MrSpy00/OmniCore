...
class SystemTrayCompanion:
    ...
    def __init__(self, router: CognitiveRouter) -> None:
        ...
        self._dashboard_url = "http://127.0.0.1:8080"
        self._last_gpu_check: float = 0.0
        self._gpu_info_cache: str | None = None
    ...
    def get_telemetry_tooltip(self) -> str:
        ...
        try:
            ...
            now = time.time()
            if now - self._last_gpu_check > 10.0:
                ...
                try:
                    ...
                    if result.returncode == 0 and result.stdout.strip():
                        ...
                        if len(gpu_info) >= 4:
                            ...
                            temp = gpu_info[3]
                            self._gpu_info_cache = f"🎮 GPU: {gpu_name} ({vram_used}/{vram_total}MB) {temp}°C"
                        else:
            ...
            if self._gpu_info_cache:
                lines.append(self._gpu_info_cache)
    ...
    def _set_permission_mode(self, mode: str) -> None:
        ...
        try:
            ...
            get_live_config().set("approval_mode", mapped_mode)
            if hasattr(self._router, "_guardian"):
                self._router._guardian.set_mode(mapped_mode)
