        ...
        def _do_query() -> None:
            try:
                if self._loop and self._loop.is_running():
                    fut = asyncio.run_coroutine_threadsafe(self._bar.execute_query(query), self._loop)
                    result = fut.result()
                else:
                    result = asyncio.run(self._bar.execute_query(query))
...
async def run_spotlight_gui(router: CognitiveRouter) -> None:
    ...
    overlay._loop = asyncio.get_running_loop()
