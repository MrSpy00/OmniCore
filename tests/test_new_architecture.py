...
async def test_dashboard_2_endpoints():
    ...
    from interfaces.dashboard import set_router
    ...
    # Graph data route (no auth required)
    r_graph = client.get("/api/graph/data")
    assert r_graph.status_code == 200
    assert "nodes" in r_graph.json()
    assert "edges" in r_graph.json()

    # Processes route requires auth - use ephemeral key
    from interfaces.dashboard import _get_dashboard_key

    key = _get_dashboard_key()
    headers = {"Authorization": f"Bearer {key}"}
    r_proc = client.get("/api/system/processes", headers=headers)
...
def test_router_hardware_adaptive_routing():
    """Test hardware adaptive routing logic without mocking the method under test."""
    from unittest.mock import MagicMock
    ...
    router._settings = MagicMock(llm_provider="ollama")
    ...
    mock_gpu_low = {"available": False}
    with (
        patch("psutil.sensors_battery", return_value=mock_battery),
        patch("core.vram_monitor.detect_running_games", return_value=[]),
        patch("tools.hardware_telemetry_toolkit._get_nvidia_gpu_info", return_value=mock_gpu_low),
    ):
    ...
    mock_gpu_high = {"available": True, "vram_total_mb": "8192", "vram_used_mb": "7500"}
    with (
        patch("psutil.sensors_battery", return_value=mock_plugged),
        patch("core.vram_monitor.detect_running_games", return_value=[]),
        patch("tools.hardware_telemetry_toolkit._get_nvidia_gpu_info", return_value=mock_gpu_high),
    ):
        route = CognitiveRouter._power_and_hardware_adaptive_route(router)
        assert route in ("gemini", "groq")

    # Test no switch needed: good battery, low GPU usage
    mock_good_battery = MagicMock(percent=80, power_plugged=True)
    mock_gpu_ok = {"available": True, "vram_total_mb": "8192", "vram_used_mb": "2048"}
    with (
        patch("psutil.sensors_battery", return_value=mock_good_battery),
        patch("core.vram_monitor.detect_running_games", return_value=[]),
        patch("tools.hardware_telemetry_toolkit._get_nvidia_gpu_info", return_value=mock_gpu_ok),
    ):
        route = CognitiveRouter._power_and_hardware_adaptive_route(router)
        # Should return None when no switch is needed
        assert route is None or route == "ollama"
