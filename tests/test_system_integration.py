...
async def test_mcp_bridge_read_write(tmp_path: Path):
    tool = SysMcpBridge()
    bridge_path = str(tmp_path / "mcp_bridge.json")
    write_out = await tool.execute(
        ToolInput(
            tool_name="sys_mcp_bridge",
            parameters={"action": "write", "server_name": "test_mcp", "config_json": "{}", "path": bridge_path},
        )
    )
...
@pytest.mark.asyncio
async def test_skill_creator_validates_syntax_and_creates_file(tmp_path: Path, monkeypatch):
    # Enable custom skills for this test
    monkeypatch.setenv("ENABLE_CUSTOM_SKILLS", "true")
    test_skills_dir = tmp_path / "skills"
    monkeypatch.setattr("tools.skill_creator_toolkit._SKILLS_DIR", test_skills_dir)
    creator = SkillCreate()
    bad_input = ToolInput(
...
@pytest.mark.asyncio
async def test_skill_creator_disabled_by_default(tmp_path: Path, monkeypatch):
    """Test that custom skills are disabled when ENABLE_CUSTOM_SKILLS is not set."""
    # Ensure ENABLE_CUSTOM_SKILLS is NOT set
    monkeypatch.delenv("ENABLE_CUSTOM_SKILLS", raising=False)
    test_skills_dir = tmp_path / "skills"
    monkeypatch.setattr("tools.skill_creator_toolkit._SKILLS_DIR", test_skills_dir)
    creator = SkillCreate()
    valid_code = """
...
"""
    ...
    res_good = await creator.execute(good_input)
    assert res_good.status.value == "failure"
    assert "disabled" in (res_good.error or res_good.result).lower()
...
async def test_mcp_server_gateway():
    ...
    # Authenticate first
    from interfaces.mcp_gateway import _get_effective_key

    auth_key = _get_effective_key()
    auth_req = json.dumps({"jsonrpc": "2.0", "id": 2, "method": "mcp_authenticate", "params": {"apiKey": auth_key}})
    auth_res = await gateway.handle_request_json(auth_req)
    auth_data = json.loads(auth_res)
    session_id = auth_data["result"]["sessionId"]

    list_req = json.dumps({"jsonrpc": "2.0", "id": 3, "method": "tools/list", "params": {"_sessionId": session_id}})
