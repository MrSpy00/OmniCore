...
def _play_audio_bytes(audio_bytes: bytes) -> None:
    """Helper to play synthesized voice audio in a separate thread."""
    try:
...
async def _run(mode: str, debug: bool = False) -> None:
    ...
    try:
        ...
        if mode == "telegram":
            ...
            gateway.set_policy_engine(router._policy)
            gateway.set_guardian(router._guardian)
            ...
            try:
                while True:
                    ...
                    if result.get("response_audio"):
                        await asyncio.to_thread(_play_audio_bytes, result["response_audio"])
