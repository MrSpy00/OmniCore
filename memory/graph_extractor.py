...
_RELATION_PATTERNS_TR: list[tuple[str, str]] = [
    (r"\b(\w+)\s+(\w+)\s+kullan[ıi](?:yor|r)", "kullanıyor"),
    (r"\b(\w+)\s+bir\s+(\w+)\s+(?:dir|tir)", "is_a"),
    (r"\b(\w+)\s+(\w+)\s+sahip\b", "has"),
    (r"\b(\w+)\s+(\w+)\s+ile\s+çalış[ıi](?:yor|r)", "works_with"),
    (r"\b(\w+)\s+(\w+)\s+ile\s+entegre\b", "integrates_with"),
    (r"\b(\w+)\s+(\w+)\s+destekl[ıi](?:yor|r)", "supports"),
    (r"\b(\w+)\s+(\w+)\s+ihtiyaç\s+(?:duyar|duymaktadır)", "requires"),
    (r"\b(\w+)\s+(\w+)\s+bağıml[ıi](?:dır|olduğu)", "depends_on"),
]
